...
class TestPlugManager(unittest.IsolatedAsyncioTestCase):
    def setUp(self) -> None:
        ...
        self._manager=PlugManager(logger, eval_time_in_min=2)
        cfg=SmartPlugConfig(id='test_controller_1', auth_user='test', auth_passwd='test', 
        ...
        cfg=SmartPlugConfig(id='test_controller_2', auth_user='test', auth_passwd='test', 
                                expected_consumption_in_watt=100, consumer_efficiency=0.5)
        self._manager._add_plug_controller("B", PlugControllerMock(logger, cfg))
        cfg=SmartPlugConfig(id='test_controller_3', auth_user='test', auth_passwd='test', 
                                expected_consumption_in_watt=50, consumer_efficiency=0.5)
    ...
    async def test_turn_on_off_sunny_day(self) -> None:
        ...
        self.assertTrue(await self._all_plugs_off(self._plug_uuids))
        await self._manager.add_smart_meter_values(0, 220, now + timedelta(minutes=2))
        self.assertTrue(await self._all_plugs_off(self._plug_uuids))
        await self._manager.add_smart_meter_values(0, 250, now + timedelta(minutes=3))
        self.assertTrue(await self._all_plugs_off(self._plug_uuids))
        await self._manager.add_smart_meter_values(0, 301, now + timedelta(minutes=4))
        ...
        await self._manager.add_smart_meter_values(80, 320, now + timedelta(minutes=5))
        self.assertTrue(await self._manager.plug('A').is_on())
        self.assertTrue(not await self._manager.plug('B').is_on())
        self.assertTrue(not await self._manager.plug('C').is_on())
        await self._manager.add_smart_meter_values(20, 380, now + timedelta(minutes=6))
        self.assertTrue(await self._manager.plug('A').is_on())
        self.assertTrue(not await self._manager.plug('B').is_on())
        self.assertTrue(not await self._manager.plug('C').is_on())
        await self._manager.add_smart_meter_values(0, 401, now + timedelta(minutes=7))
        self.assertTrue(await self._manager.plug('A').is_on())
        self.assertTrue(not await self._manager.plug('B').is_on())
        self.assertTrue(not await self._manager.plug('C').is_on())
        await self._manager.add_smart_meter_values(0, 460, now + timedelta(minutes=8))
        self.assertTrue(await self._manager.plug('A').is_on())
        self.assertTrue(await self._manager.plug('B').is_on())
        self.assertTrue(not await self._manager.plug('C').is_on())
        await self._manager.add_smart_meter_values(10, 490, now + timedelta(minutes=9))
        self.assertTrue(await self._manager.plug('A').is_on())
        self.assertTrue(await self._manager.plug('B').is_on())
        self.assertTrue(not await self._manager.plug('C').is_on())
        await self._manager.add_smart_meter_values(0, 600, now + timedelta(minutes=10))
        self.assertTrue(await self._all_plugs_on(self._plug_uuids))
        await self._manager.add_smart_meter_values(90, 500, now + timedelta(minutes=11))
        self.assertTrue(await self._manager.plug('A').is_on())
        self.assertTrue(await self._manager.plug('B').is_on())
        self.assertTrue(not await self._manager.plug('C').is_on())
        await self._manager.add_smart_meter_values(30, 480, now + timedelta(minutes=12))
        self.assertTrue(await self._manager.plug('A').is_on())
        self.assertTrue(await self._manager.plug('B').is_on())
        self.assertTrue(not await self._manager.plug('C').is_on())
        await self._manager.add_smart_meter_values(100, 400, now + timedelta(minutes=13))
        self.assertTrue(await self._manager.plug('A').is_on())
        self.assertTrue(not await self._manager.plug('B').is_on())
        self.assertTrue(not await self._manager.plug('C').is_on())
        await self._manager.add_smart_meter_values(0, 400, now + timedelta(minutes=14))
        self.assertTrue(await self._manager.plug('A').is_on())
        self.assertTrue(not await self._manager.plug('B').is_on())
        self.assertTrue(not await self._manager.plug('C').is_on())
        await self._manager.add_smart_meter_values(50, 350, now + timedelta(minutes=15))
        self.assertTrue(await self._manager.plug('A').is_on())
        self.assertTrue(not await self._manager.plug('B').is_on())
        self.assertTrue(not await self._manager.plug('C').is_on())
        await self._manager.add_smart_meter_values(110, 290, now + timedelta(minutes=16))
        self.assertTrue(await self._all_plugs_off(self._plug_uuids))
        await self._manager.add_smart_meter_values(0, 190, now + timedelta(minutes=17))
        self.assertTrue(await self._all_plugs_off(self._plug_uuids))
        await self._manager.add_smart_meter_values(100, 90, now + timedelta(minutes=18))
        self.assertTrue(await self._all_plugs_off(self._plug_uuids))

    async def test_turn_on_strong_production_increase(self):
        now = datetime.now()
        await self._manager.add_smart_meter_values(200, 0, now)
        self.assertTrue(await self._all_plugs_off(self._plug_uuids))
        await self._manager.add_smart_meter_values(100, 100, now + timedelta(minutes=1))
        self.assertTrue(await self._all_plugs_off(self._plug_uuids))
        await self._manager.add_smart_meter_values(0, 300, now + timedelta(minutes=2))
        self.assertTrue(await self._all_plugs_off(self._plug_uuids))
        ...
        await self._manager.add_smart_meter_values(0, 300, now + timedelta(minutes=3))
        # Plug should be turned when continuously having overproduction
        # Behind the scenes the break-even point gets decreased in this case
        self.assertTrue(await self._manager.plug('A').is_on())
        state_a = await self._manager.plug('A').state
